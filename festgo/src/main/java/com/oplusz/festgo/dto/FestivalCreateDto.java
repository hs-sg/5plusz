...
public class FestivalCreateDto {
	...
	public Festival toFestivalEntity() {
		return Festival.builder().feName(fName).feStartDate(fStartDate).feEndDate(fEndDate).lcId(lcId)
				.feAddress(fAddress).fePhone(fPhone).meSponsor(mSponsor).feFee(fFee).theId(theId).feContents(fContents)
				.feHomepage(fHomepage).feImageMain(fImageMain).fePoster(fPoster).feCreatedTime(fCreatedTime)
				.feModifiedTime(fModifiedTime).build();
	}
